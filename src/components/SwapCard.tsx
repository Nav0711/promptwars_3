  ...
  return (
    <div className="glass-panel rounded-2xl p-5 space-y-4 relative overflow-hidden"
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {!activeSwap && !loading && (
          <button onClick={fetchSuggestion} className="p-1.5 rounded-lg transition-all ripple"
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <button onClick={fetchSuggestion} className="btn-primary text-xs py-2 px-4">Get a Swap</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
